...
export const sendFriendRequest = async (senderId: string, receiverId: string) => {
    ...
    if (existingFriendship) {
        throw new AppError('Friend request already sent or you are already friends.', 409);
    }
...
export const respondToFriendRequest = async (responderId: string, requesterId: string, status: FriendshipStatus) => {
    ...
    if (!friendship || friendship.status !== 'pending' || friendship.actionUserId === responderId) {
...
export const getFriends = async (userId: string) => {
    ...
    return friendships.map(f => (f.userOneId === userId ? f.userTwo : f.userOne));
};
