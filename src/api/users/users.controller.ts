...
export const searchUsers = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const users = await usersService.searchUsers(req.query.query as string, req.user!.id);
...
export const sendFriendRequest = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const friendship = await usersService.sendFriendRequest(req.user!.id, req.body.userId);
...
export const respondToFriendRequest = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const friendship = await usersService.respondToFriendRequest(req.user!.id, userId, status);
